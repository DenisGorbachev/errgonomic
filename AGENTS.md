...
[tasks."gen:docs"]
# internal/repomix.md includes code files, so wait for concurrent code fixes without making them a prerequisite for direct generation.
wait_for = ["fix:code"]
run = [{ task = "repomix", args = ["internal/repomix.md"] }]
